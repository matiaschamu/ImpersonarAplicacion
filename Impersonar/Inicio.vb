...
Public Class Inicio
    ...
    Private Sub Inicio_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        Try
            Dim ArchivoConfig As String = My.Application.Info.DirectoryPath & "\ImperConfig.xml"
            If System.IO.File.Exists(ArchivoConfig) Then
                ...
                If System.IO.File.Exists(mPathAplicacion) Then
                    ...
                    'Dim II As New BibliotecaMAF.Clases.Security.ImpersonateUser
                    'II.Impersonate("", mUsuario, mPassword)

                    Dim Pass As New System.Security.SecureString
                    For i = 1 To mPassword.Length
                        Pass.AppendChar(CChar(Mid(mPassword, i, 1)))
                    Next

                    Dim Start As New ProcessStartInfo(mPathAplicacion, mLineaComandos)
                    Start.Domain = My.Computer.Name
                    Start.UserName = mUsuario
                    Start.Password = Pass
                    Start.ErrorDialog = True
                    Start.ErrorDialogParentHandle = Me.Handle
                    ...
                    Dim D As DirectoryInfo = System.IO.Directory.GetParent(mPathAplicacion)
                    Start.WorkingDirectory = D.FullName
                    System.Threading.Thread.Sleep(CInt(mRetardo))

                    Try
                        Process.Start(Start)
                    Catch ex As Exception
                        MsgBox(ex.Message)
                        MsgBox("Si el archivo que desea abrir esta marcado para ejecutarse como administrador (un escudo en el icono) pruebe sacandole esta opcion")
                    Finally
                        'II.Undo()
                    End Try
                End If
    ...
    Private Sub ButtonGuardar_Click(sender As System.Object, e As System.EventArgs) Handles ButtonGuardar.Click
        ...
        Xx.Save(My.Application.Info.DirectoryPath & "\ImperConfig.xml")
        Me.Close()
    End Sub
